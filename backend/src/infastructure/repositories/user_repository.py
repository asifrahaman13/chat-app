...
from config.config import HOST, DBNAME, PASSWORD
...
class UserRepository:
    ...
    def save_data(self, user_id, session_id, session_data):
        try:
            ...
            with Session(self.engine) as session:
                user = session.exec(
                    select(UserSessionData).where(UserSessionData.session_id == session_id)
                ).first()
                if user:
                    ...
                    new_user_data = UserSessionData(
                        session_id=session_id, session_data=session_data_append
                    )
